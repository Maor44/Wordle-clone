...
const guessGridContainer = document.querySelector("[data-guess-grid]");
...
function createGrid() {
    Array.from({length: 30}).forEach(_ => {
        const tile = document.createElement("div");
        tile.classList.add("tile");
        guessGridContainer.append(tile);
    })
}

function createKeyboard() {
    keyboardsKeys.forEach((k, index) => {
        const button = document.createElement('button');
        button.textContent = k.toUpperCase();
        button.dataset.key = k.toUpperCase();
        if (k === 'enter' || k === 'delete') {
...
function handleMouseClick(e) {
    ...
    if (e.target.matches("[data-key='DELETE']")) {
        deleteKey();
        return;
    }
...
function pressKey(key) {
    ...
    const nextTile = guessGridContainer.querySelector(":not([data-letter])");
...
function showAlert(message, duration = 800) {
...
function flipTiles(tile, index, array, guess) {
    ...
    const key = keyboardContainer.querySelector(`[data-key="${letter}"i]`);
    setTimeout(() => {
...
function checkWinLose(guess, tiles) {
    ...
    const remainingTiles = guessGridContainer.querySelectorAll(":not([data-letter])");
